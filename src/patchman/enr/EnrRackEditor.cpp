    ...
    , model_(new EnrRackModel(rack_, this))
{
    ...
    connect(model_, &EnrRackModel::dataChanged, [this]() { Q_EMIT(dataChanged()); });
    connect(
        table_->selectionModel(),
        &QItemSelectionModel::selectionChanged,
        this,
        &EnrRackEditor::selectionChanged);
    setModuleRowSpans();
}
...
void EnrRackEditor::selectCircuits(const QList<unsigned int> &circuits)
{
    QItemSelection s;
